...
> [!IMPORTANT]
> `--host`, `--ports`, `--dns` и `--mtu` действуют **только на первой установке**.
> На уже настроенном сервере адрес, порты, подсети, DNS и MTU берутся из
> `/etc/amnezia/amneziawg/services.env` и не меняются — в том числе вместе с
> `--reconfigure`, где домен спрашивается, проверяется резолвом, а потом
> затирается сохранённым. Чтобы сменить домен или порт, правь `services.env`
> руками, перезапускай юниты и перевыпускай клиентов (`awg-client regen-all`).
...
Наборов 8. Прогон **безопасен на боевой машине**: ничего не устанавливается и
