...
Приложение 5.0.1.5 подписывает наш конфиг 3.0 как «AmneziaWG (version 3.1)» — и это не описка. Начиная с 5.0 клиент перестал верить полю `protocol_version` из ссылки и считает версию сам: любой непустой параметр слоя 3 — у нас это `HeaderProtectionKey` — сразу даёт тройку, а знает он ровно одну, `3.1`; строки «3.0» в приложении нет вообще. Сборки 4.9.x той же ссылке по-прежнему напишут «version 2». На работу это не влияет ни там, ни там — параметры 3.0 передаются и применяются полностью.
