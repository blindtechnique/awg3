...
### 3.0 в ядре: что будет дальше

Апстрим готовит поддержку 3.0 прямо в kernel-модуле — [PR #192](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/pull/192), ветка `feat/awg3`. Когда её вольют, слой 3.0 можно будет перевести с userspace-демона на ядро и получить скорость слоя 2.0 при обфускации 3.0.

Формат конфигов у нас уже совпадает с тем, что ждёт эта ветка: ключ header protection в base64 внутри `.conf` и в hex через UAPI, диапазоны вида `118-134`, ChaCha20 с 32-байтным ключом и 12-байтным nonce. **Переход не потребует перевыпуска клиентов** — меняется только датапас на сервере.

Проверить, доехала ли поддержка до релиза, можно в любой момент:

```bash
awg-upstream-check
```

Скрипт смотрит не на номер PR, а на наличие атрибута header protection в `master` модуля, и сам сообщит, когда переключаться станет можно.

### Экспериментальный режим ядра

Попробовать 3.0 на ядре уже сейчас:

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/blindtechnique/awg3/main/install.sh) --kmod3
```

Установщик соберёт модуль и утилиты из ветки `feat/awg3`, переведёт слой 3.0 на `awg-quick` и положит параметры 3.0 прямо в `[Interface]`. Профиль обфускации при этом **не перегенерируется** — выданные клиенты остаются рабочими. Вернуться обратно:

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/blindtechnique/awg3/main/install.sh) --no-kmod3
```

> [!WARNING]
> Ветка не влита в master и правится почти ежедневно. На 30 июля 2026 в ней есть поломка: политика netlink в модуле объявляет `WGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL` как `NLA_U64`, а утилиты шлют `u32` — из-за этого **любой peer с `PersistentKeepalive` отвергается с «Invalid argument»**, и клиенты не подключаются на обоих слоях. `awg-doctor` проверяет это отдельно и прямо об этом говорит.
>
> Второе, важнее: **модуль нельзя подменить на работающей системе.** Пока он загружен, утилиты и модуль обязаны быть из одного набора. Если версии разойдутся, `awg` не выдаёт ошибку, а зависает намертво — процессы уходят в состояние `D`, счётчик ссылок модуля ломается, и спасает только перезагрузка. Установщик выгружает модуль сам, а если не получается — останавливается и просит перезагрузиться. **Планируй один reboot на каждое переключение режима.**
>
> Режим годится для экспериментов на тестовом сервере, не на боевом.
...
| `--kmod3` / `--no-kmod3` | [экспериментальный режим ядра](#экспериментальный-режим-ядра) для слоя 3.0 и возврат из него |
